    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                ...
                <span class="ms-2 text-sm text-gray-600 dark:text-gray-400">{{ __('Запомни ме') }}</span>
            </label>
        </div>

        <!-- Actions Row -->
        <div class="flex items-center justify-center mt-2 flex-wrap gap-3">
            ...
            <div class="flex items-center justify-between mt-2 flex-wrap gap-3">
                ...
                <a href="{{ route('password.request') }}" class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800">
                    {{ __('Заборавена лозинка?') }}
                </a>
                @endif
            </div>
            <div class="text-center">
                <x-primary-button class="ms-auto mt-3">
                    {{ __('Логирај се') }}
                </x-primary-button>
            </div>
